...
public class B08_TypeCasting {
	...
	public static void main(String[] args) {
		...
		// 1. 자연스러운 타입 캐스팅
		//4byte에 1byte 값을 넣을 때는 알아서 타입이 변한다 (byte -> int)
		_int =_byte;
		System.out.println(_int);
		
		// 2. 값이 손상될 수도 있는 타입캐스팅
		// 더 넓은 범위의 값을 작은 변수에 넣을 때는 값이 손상될 우려가 있어 컴파일 에러가 발생한다
		// 개발자가 강제로 타입캐스팅해야 컴파일이 가능해진다
		_int = 200;
		_byte = (byte)_int; 		
		...
		System.out.println(a/b);// 출력만 이렇게 하는거지 내부의 계산 값은 변하지 않음
