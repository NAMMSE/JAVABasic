...
class Domain implements Comparable <Domain>{
	...
	public int compareTo(Domain d) {
		return d.count.compareTo(count); // 내림차순 정렬
	}
	
	public String toString() {
		return String.format("[%-20s / %3d]", domain, count);
	}
...
public class D09_RegexEmail {
	...
	/*
	  1. emails 문자열에 있는 모든 이메일들을 ArrayList에 저장하기
	  
	  2. 어떤 도메인(chosun.com , munhwa.com)이 몇개씩 있는지 세어서 출력해보세요 // quiz에 countfruits 확인하기
	  
	  3. 모든 도메인들을 개수 기준 내림차순 정렬하여 출력
	 */
	...
	public static void main(String[] args) {
		...
		Pattern emailPattern = Pattern.compile("(\\w+)@(\\w+\\.\\w+(\\.\\w+)*)"); // 도메인 굳이 안만들고 이메일 뒷부분이 도메인이니까 그룹으로 만들어줌
		//Pattern domainPattern = Pattern.compile("@\\w+\\.\\w+(\\.\\w+)*");// 도메인 패턴 만들기
		Matcher emailMatcher = emailPattern.matcher(emails);
		
		List<String> emailList = new ArrayList<>();
		Map<String, Domain> domainCountMap = new HashMap<>(); // 그냥 Map이랑 HashMap 차이 확인해봐야됨 // 도메인 자리에 원래 integer였음
		
		
		while(emailMatcher.find()) {
			emailList.add(emailMatcher.group());
			
			String domainName = emailMatcher.group(2);
			Domain domain = domainCountMap.putIfAbsent(domainName, new Domain(domainName)); // 
			if(domain != null) { 
				domain.plus();
			}
			
//			System.out.println("id : "+emailMatcher.group(1));
//			System.out.println("domail : "+emailMatcher.group(2));
		}
		
		System.out.println(emailList);
		
		List<Domain> domainList = new ArrayList<>(domainCountMap.values()); // 이거 확인
	...
	static String emails = "조선일보	가정/육아	박은주	zeeny@chosun.com\r\n"
	...
	// 내코드
//	public static void main(String[] args) {
//		ArrayList<String> emaillist = new ArrayList<>(); // 이메일을 저장할 어레이 리스트 생성
//		Pattern emailPattern = Pattern.compile("\\w+@\\w+\\.+[a-zA-Z]+(.[a-zA-Z]+)*");  // 패턴 타입 인스턴스에 이메일 형식의 문자열을 찾는 컴파일 메서드 선언
//		Matcher matchResult = emailPattern.matcher(emails); // 매쳐 타입 인스턴스 생성 / 패턴 타입 인스턴스로 문자열 검사
//		while(matchResult.find()) {
//			emaillist.add(matchResult.group());
//		}
////		for(int i=0;i<emaillist.size();i++)
////			System.out.printf("%s\n",emaillist.get(i));
//		
//		ArrayList<String> domainlist = new ArrayList<>();
//		Pattern domainPattern = Pattern.compile("\\w+[.]+[a-zA-Z]{2,4}+([.][a-zA-Z]{2,4})*");  // 패턴 타입 인스턴스에 이메일 형식의 문자열을 찾는 컴파일 메서드 선언
//		Matcher domainResult = domainPattern.matcher(emails);
//		int cnt=0;
//		while(domainResult.find()) {
//			domainlist.add(domainResult.group());
//			
//	
//		}
//		for(int i=0;i<emaillist.size();i++)
